...
void renderObject(const GameObject& obj, const Camera& camera) {
    // 1. Transform the object's vertices:
    ...
    for (auto& p : transformedPoints) {
        ...
        // --- Apply Rotation (using your existing rotate function) ---
    ...
    // 2. Project and draw (similar to your previous backfaceCullAndDraw)
    ...
    void input() { //Now handles input in screen.h to not crowd main
        SDL_Event event;
